...
else patch -p1 < "$HERE/patches/gutenprint-5.3.4-cardtray.patch" || { echo "!! patch failed — see tools/apply-patch.py" >&2; exit 1; }
fi
# Give the build a HIGHER version than the archive (…+cardtray24). Without this the packages
...
s=(f"{pkg} ({ver}+cardtray24) {dist}; urgency=medium\n\n  * Local build: Canon CD-tray patch for PVC card trays "
   f"(PageSize=TrayJ for the MX920, StpCDNoMask, CD X/Y adjustment ±120pt). https://github.com/jphein/gutenprint-cardtray\n\n"
